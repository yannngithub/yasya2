...
NOTE : Pengen limitnya unlimited?, Bisa kirim .buypremium`);
          found = true;
        }
      }
      if (found === false) {
...
🔗 Media Url : ${result.image}`,
      ...
      case "play":
        ...
        await yts(text).then(async (yut) => {
          let quality = "128kbps";
          let media = await yta(yut.all[0].url, quality);
...
🔗 Url : ${i.url}\n\n─────────────────\n\n`;
      ...
      case "ig":
        ...
        m.reply(mess.wait);
        igdown = await fetchJson(`https://api.xyroinee.xyz/api/downloader/instagram?url=${text}=&apikey=GTjLO958RE`);
        igdown1 = igdown.data[0].url;
        ichi.sendMessage(from, {
          video: { url: `${igdown1}` },
          mimetype: "video/mp4",
          quoted: m,
...
*Caption:* ${result.title}`,
      ...
      case "listgrupbot":
        ...
        break;
      case "hooh":
        let test = await store.chats.all().map((v) => v.id);
        let test1 = await store.loadMessages.all();
        console.log(test1);
        break;
