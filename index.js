...
async function startIchigo() {
  ...
  cfonts.say("YaSya BOT", {
    font: "block",
    gradient: ["red", "magenta"],
    align: "center",
  });
  cfonts.say("Whatsapp Bot By Yannn", {
    font: "console",
    gradient: ["red", "magenta"],
    align: "center",
  });
  ...
  try {
    let ichi = await makeWASocket({
      logger: pino({ level: "silent" }),
      printQRInTerminal: true,
      browser: ["YaSya - Bot", `Chrome`, "109.0.5414.120"],
    ...
    ichi.ev.on("connection.update", async (update) => {
      ...
      try {
        ...
        if (
          ...
          await ichi.sendMessage(gruplog, { text: "YaSya BOT telah aktif" });
        }
        if (connection === "close") {
    ...
    ichi.ev.on("group-participants.update", async (anu) => {
      ...
      try {
        ...
        for (let num of participants) {
          ...
          try {
            if (anu.action == "add" && num.includes(ichi.user.id)) {
              ichi.sendMessage(gruplog, {
                text: `ID Grup ${metadata.subject} adalah: \n${anu.id}\n\nJangan lupa kirim ke Owner BOT ya:)`,
              });
            }
          } catch {
            if (anu.action == "add" && num.includes(ichi.user.id)) {
          ...
          /*if (anu.action == "add") {
            tekswell = `*Selamat datang @${num.split("@")[0]} di Grup ${
              metadata.subject
            } 👋*\n\nJangan lupa baca rules grup\nKetik Help untuk menggunakan BOT`;
            ichi.sendMessage(anu.id, {
              image: { url: ppuser },
    ...
    //add detek Telepon by Alvianto
    ichi.ev.on("call", async (senku) => {
      for (let sen of senku) {
        if (sen.status == "offer") {
          const obj = {
            id: sen.chatId,
            alasanban: "Telepon BOT",
          };
          readbanned.push(obj);
          fs.writeFileSync(
    ...
    /*ichi.ev.on("messages.reaction", async (pesan) => {
      try {
        ...
        conn = ichi;
        if (
          m.reaction.key.id.startsWith("BAE5") &&
          m.reaction.key.id.length === 16
        )
          return;
        let mesg = await store.loadMessage(
          m.reaction.key.remoteJid,
          m.key.id,
          conn
        );
        let frem = m.reaction.key.remoteJid
          ? m.reaction.key.participant
          : m.reaction.key.remoteJid;
        let frum = m.key.remoteJid ? m.key.participant : m.key.remoteJid;
        await ichi.sendMessage(
          m.reaction.key.remoteJid,
          {
            text: `*【﻿ DETECT REACTION MESSAGE 】*\n\n*_Tagged:_* @${
              (m.reaction.key.fromMe
                ? ichi.decodeJid(ichi.user.id)
                : ichi.decodeJid(frem)
              ).split("@")[0]
  ...
  vcard: `BEGIN:VCARD\nVERSION:3.0\nN:Yannn\nFN:Yannn Owner Bot\nitem1.TEL;waid=62895401223315:+62895401223315\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:tuyulheroes11@gmail.com\nitem2.X-ABLabel:Email\nitem3.URL:https://instagram.com/alvianto17\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
  })
  }
  ichi.sendMessage(jid, { contacts: { displayName: `1 Kontak`, contacts: list }, ...opts }, { quoted })
  }*/
